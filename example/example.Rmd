---
title: "Example Machine Readable Hypothesis"
output: 
  html_document:
    toc: true
    toc_float: true
...
study <- add_analysis(study,
         ...
         add_analysis(id = "recip_analysis",
                      code = t.test(
                        kin$recip_self,
                        kin$recip_other,
                        paired = TRUE,
...
```{r}
study_report(study, "prereg", "prereg.html")
```

### add_data

The `id` you give the data can be used in the analysis code to refer to this dataset. The `data` argument can be a data frame or a file path to a data file or a PsychDS-formatted codebook. You can optionally add column descriptions with the argument `coldesc`. 
...
study <- add_data(study, id = "kin", data = data, coldesc = desc)
...
```{r}
study_save(study, "postreg.json")
study_report(study, "postreg", "postreg.html")
```
