...
\hypersetup{
  ...
  pdfauthor={Daniël Lakens~\& Lisa M. DeBruine},
...
Daniël Lakens \url{https://orcid.org/0000-0002-0247-239X}
...
Another benefit of making statistical hypotheses machine readable is that many important aspects of the hypothesis test become accessible, findable, and usable. This will benefit researchers in the future. We can imagine a utopian future where meta-data files such as the example in Box 1 are accessible by browsing to a website that consists of the DOI, appended by /meta (e.g., \url{https://doi.org/10.1098/rspb.2002.2034/meta/}). Researchers can access these files to load all the information that is available about statistical predictions. For example, when a completely reproducible workflow is used, and data can be accessed as part of the JSON file, the meta-data file should be sufficient to easily calculate or access effect sizes from the performed statistical tests for meta-analyses.
...
Scienceverse illustrates one possible workflow where after specifying the hypotheses at a Stage 1 submission, a machine readable html report can be produced with the \texttt{study\_save} function. This report looks similar to Figure 1, without any of the lines containing coloured true or false evaluations of the predictions. When the data is collected, reviewers should be able to use the JSON file generated at Stage 1, load the collected data with the \texttt{add\_data} function, run the preregistered analyses, and generate a human readable report as in Figure 1. This should make it realtively easy for reviewers to compare if planned and reported analyses.
